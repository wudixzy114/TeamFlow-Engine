...
async def get_focus_time_data(
    period: PeriodLiteral = Query(default="week"),
    team: models.Team = Depends(get_team_and_verify_membership),
    db: AsyncSession = Depends(get_db),
):
    """提取团队指定周期内的专注时长统计信息。"""
    return await crud.get_team_focus_time_data(db=db, team_id=str(team.id), period=period)
...
async def get_insights_data(
    ...
    return await crud.get_team_insights_data(db=db, team_id=str(team.id), period=period)