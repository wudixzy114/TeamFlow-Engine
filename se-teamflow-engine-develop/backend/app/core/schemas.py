...
class UserSkillModify(BaseModel):
...
class NewTeamChat(BaseModel):
    content : str
    tag : str
    model_config = ConfigDict(from_attributes=True)
    
class NewTeamChatID(BaseModel):
    id : str
...
class ForumSectionModify(BaseModel):
    name: Optional[str] = None
    description: Optional[str] = None

class ForumSection(ForumSectionBase):
    id: str
    team_id: str
    created_at: datetime
...
class ForumPostModify(BaseModel):
...
class ForumPostResponse(ForumPostBase):
    ...
    likes: List[Any] = Field(default=[], exclude=True)
